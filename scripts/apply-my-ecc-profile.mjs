...
if (!fs.existsSync(PROFILE) || !fs.existsSync(VERSION)) {
  fail('Profile or VERSION file is missing.');
}
...
for (const key of ['skills', 'agents', 'commands']) {
  ...
  if (new Set(profile[key]).size !== profile[key].length) {
    fail('Profile field ' + key + ' contains duplicates.');
  }
}

if (!['minimal', 'standard', 'strict'].includes(profile.hook_profile)) {
  fail('Unsupported hook profile: ' + profile.hook_profile);
}

const missing = [];
for (const item of profile.skills) {
  if (!fs.existsSync(path.join(ROOT, 'skills', item, 'SKILL.md'))) {
    missing.push('skills/' + item + '/SKILL.md');
  }
}
for (const item of profile.agents) {
  if (!fs.existsSync(path.join(ROOT, 'agents', item + '.md'))) {
    missing.push('agents/' + item + '.md');
  }
}
for (const item of profile.commands) {
  if (!fs.existsSync(path.join(ROOT, 'commands', item + '.md'))) {
    missing.push('commands/' + item + '.md');
  }
}
...
if (CHECK) {
  if (!fs.existsSync(MARKETPLACE)) fail('marketplace.json is missing.');
  if (fs.readFileSync(MARKETPLACE, 'utf8') !== generated) {
    fail('marketplace.json is stale; regenerate it with this script.');
  }
  console.log('[my-ECC] Valid for ECC ' + eccVersion + ' (' + pluginVersion + ').');
} else {
  fs.writeFileSync(MARKETPLACE, generated);
  console.log('[my-ECC] Generated marketplace for ECC ' + eccVersion + ' (' + pluginVersion + ').');
}
